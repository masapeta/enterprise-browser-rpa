...
PLANNER_PROMPT = """
You are an autonomous browser agent. Your goal is to complete the user's task by navigating the web.
...
class Planner:
    async def plan(self, history: List[Dict], context: str) -> Dict[str, Any]:
        messages = [
            {"role": "system", "content": PLANNER_PROMPT.format(history=json.dumps(history[-5:]), context=context)},
